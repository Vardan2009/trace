...
void tracefs_read_header_sector(void) {
    ...
    tracefs_header_sector =
        (tracefs_header_sector_t *)tracefs_boot_sector_buffer;
}

int tracefs_read_file_from_path(const char *path, uint8_t *buffer,
                                uint32_t buffer_sz) {
    char dirpath[32];
    char filename[16];
...
int tracefs_list_directory(const char *dirpath, fs_entry_t dirs[256]) {
    ...
    for (int i = 1;; ++i) {
        ...
        if (strcmp(dirpath, f_entry->directory) == 0) {
            ...
            dirs_counter++;
        } else if (strncmp(f_entry->directory, normalized_dirpath,
                           dirpath_len) == 0) {
            char subdir[32];
            int j = 0;
...
int tracefs_dir_exists(const char *dirpath) {
    ...
    for (int i = 1;; ++i) {
        ...
        if (strncmp(dirpath, f_entry->directory, strlen(dirpath)) == 0)
            return 1;
    }
    return 0;
}
...
fs_info_t tracefs_info() {
    return (fs_info_t){TRACEFS, NULL, 0, -1};
}