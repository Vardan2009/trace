...
typedef struct {
    ...
    uint32_t vol_space_size_lsb;  // Offset 80–83: Volume Space Size (LSB part,
                                  // little-endian)
    uint32_t vol_space_size_msb;  // Offset 84–87: Volume Space Size (MSB part,
                                  // big-endian ignored on x86)
    ...
    int16_t
        vol_seq_num_lsb;  // Offset 124–125: Volume Sequence Number (LSB part)
    int16_t
        vol_seq_num_msb;  // Offset 126–127: Volume Sequence Number (MSB part)
    int16_t
        logical_block_sz_lsb;  // Offset 128–129: Logical Block Size (LSB part)
    int16_t
        logical_block_sz_msb;  // Offset 130–131: Logical Block Size (MSB part)
    ...
    uint8_t root_dir[34];  // Offset 156–189: Directory Record for the root
                           // directory
    ...
    char data_preparer_id[128];  // Offset 446–573: Data Preparer Identifier
                                 // (strA)
    char application_id[128];  // Offset 574–701: Application Identifier (strA)

    /* 702–812: File Identifier Strings */
    char copyright_file_id[37];  // Offset 702–738: Copyright File Identifier
                                 // (strD)
    char abstract_file_id[37];   // Offset 739–775: Abstract File Identifier
                                 // (strD)
    char bibliographic_file_id[37];  // Offset 776–812: Bibliographic File
                                     // Identifier (strD)

    /* 813–880: Date and Time fields (in dec-datetime format) */
    char
        vol_creation_date[17];  // Offset 813–829: Volume Creation Date and Time
    ...
    /* 883–1394 */
    uint8_t app_use[512];  // Offset 883–1394: Application Use (contents not
                           // defined by ISO 9660)
